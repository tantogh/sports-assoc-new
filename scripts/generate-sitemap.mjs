...
// （src/app/articles/[category]/.../page.tsx と揃える）
...
// サイトマップから除外する静的ルート
// - /archives/articles: 過去記事の一覧ページ（個別の過去記事とあわせて対象外にする）
const EXCLUDED_STATIC_ROUTES = new Set(["/archives/articles"]);
...
function collectStaticRoutes() {
  ...
  for (const page of pages) {
    ...
    const route = "/" + visible.join("/");
    if (EXCLUDED_STATIC_ROUTES.has(route)) continue;
    routes.push(route);
  }
...
/**
 * content/articles/ 以下の Markdown から記事ルートを収集する。
 *   content/articles/<cat>/<y>/<m>/<n>.md          -> /articles/<cat>/<y>/<m>/<n>
 * lastmod は frontmatter の date、無ければファイルの更新日時。
 *
 * content/archives/ は過去記事のためサイトマップには含めない。
 */
function collectArticleRoutes() {
  const entries = [];

  const files = fg.sync("articles/*/*/*/*.md", { cwd: CONTENT_DIR });
  for (const file of files) {
    ...
    entries.push({
      route: `/articles/${category}/${year}/${month}/${number}`,
      lastmod,
    });
  }
...
function priorityFor(route) {
  if (route === "/") return "1.0";
  if (route.startsWith("/articles/")) return "0.5";
  return "0.7";
}
